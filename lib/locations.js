...
router.get('/', (req, res, next) => {
    ...
    Location.find()
        ...
        .select('_id name city likes ')//address city description category raggiungibilita locationImage photoDesc hour date likes')
        .exec()
        .then(docs => {
            ...
            res.status(200).json({
                ...
                locations: docs.map(doc => {
                    return {
                        ...
                        hour: doc.hour,
                        date: doc.date, //Il resto delle informazioni viene dato nella pagina della singola location*/
                        likes: doc.likes,
                        get: {
                            type: 'GET',
...
router.get('/:locationId', (req, res, next) => {
    ...
    } /*else {                                          //possiamo togliere questa parte perchè
        res.status(400).json({                          //se facciamo GET di locations/ semplicemente 
            error: 'Invalid location ID'                //finisce nel caso GET di tutte le locations
        });
    }*/
});
...
router.delete('/:locationId', (req, res, next) => {
    ...
    .exec()
    .then(result => {
        /*fs.unlink(result.locationImage, (err) => {
            if(err) {
                console.log(err);
                return
            }
